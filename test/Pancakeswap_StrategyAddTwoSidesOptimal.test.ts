...
describe('Pancakeswap - StrategyAddTwoSidesOptimal', () => {
  ...
  beforeEach(async () => {
    ...
    vault = await upgrades.deployProxy(Vault, [
      config.address, baseToken.address, 'Interest Bearing BTOKEN', 'ibBTOKEN', 18, debtToken.address
    ]) as Vault;
    ...
    addStrat = await upgrades.deployProxy(StrategyAddTwoSidesOptimal, [router.address, vault.address]) as StrategyAddTwoSidesOptimal
