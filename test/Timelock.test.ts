...
describe("Timelock", () => {
  ...
  beforeEach(async () => {
    ...
    configurableInterestVaultConfigAsAlice = ConfigurableInterestVaultConfig__factory.connect(configurableInterestVaultConfig.address, alice);
  ...
  context('when non-owner try to adjust params', async () => {
    ...
    it('should not allow to do so on Worker contract', async () => {
      await expect(
        pancakeswapWorkerAsAlice.setReinvestBountyBps(100)
      ).to.be.revertedWith('Ownable: caller is not the owner');
      await expect(
        pancakeswapWorkerAsAlice.setStrategyOk([ADDRESS0], true)
      ).to.be.revertedWith('Ownable: caller is not the owner');
      await expect(
        pancakeswapWorkerAsAlice.setCriticalStrategies(ADDRESS0, ADDRESS0)
      ).to.be.revertedWith('Ownable: caller is not the owner');
    });
    ...
    it('should not allow to do so on ConfigurableInterestVaultConfig', async () => {
      ...
      await expect(
        configurableInterestVaultConfigAsAlice.setWorkers(
          [ADDRESS0], [ADDRESS0]
        )
      ).to.be.revertedWith('Ownable: caller is not the owner');
    });
    ...
    it('should not allow to do so on WorkerConfig', async () => {
      await expect(
        pancakeswapWorkerConfig.setOracle(ADDRESS0)
      ).to.be.revertedWith('Ownable: caller is not the owner');
  ...
  context('when timelock adjust params', async () => {
    ...
    it('should set oracle in WorkerConfig', async () => {
      const eta = (await TimeHelpers.latest()).add(TimeHelpers.duration.days(ethers.BigNumber.from('4')));
      await timelockAsAdmin.queueTransaction(
        pancakeswapWorkerConfig.address, '0', 'setOracle(address)',
        ethers.utils.defaultAbiCoder.encode(
          ['address'],
