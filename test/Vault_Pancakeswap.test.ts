...
describe('Vault - Pancake', () => {
  ...
  beforeEach(async () => {
    ...
    fairLaunch = await FairLaunch.deploy(
      alpacaToken.address, (await alice.getAddress()), ALPACA_REWARD_PER_BLOCK, 0, ALPACA_BONUS_LOCK_UP_BPS, 0
    );
  ...
  context('when user open position', async() => {
    ...
    it('should not affect other users when someone does emergencyWithdraw', async () => {
      ...
      expect(await alpacaToken.balanceOf(await alice.getAddress())).to.be.bignumber.eq(ALPACA_REWARD_PER_BLOCK.mul(blockAliceAddLoan.sub(blockAliceLoan)));

      let aliceBefore = await baseToken.balanceOf(await alice.getAddress());
      let aliceAlpacaBefore = await alpacaToken.balanceOf(await alice.getAddress());
      console.log("Bob pending ALPACA: ", (await fairLaunch.pendingAlpaca(0, await bob.getAddress())).toString())
      console.log("Alice pending ALPACA: ", (await fairLaunch.pendingAlpaca(0, await alice.getAddress())).toString())
      // Alice close position#2
      await vaultAsAlice.work(
        2,
        pancakeswapWorker.address,
      ...
      // Hence Alice will get 11 * ALPACA_REWARD_PER_BLOCK as it is passed 11 blocks
      expect(aliceAfter).to.be.bignumber.gt(aliceBefore);
      expect(aliceAlpacaAfter.sub(aliceAlpacaBefore)).to.be.bignumber.eq(ALPACA_REWARD_PER_BLOCK.mul(blockAliceClose.sub(blockAliceLoan)));
    }).timeout(50000)
  });
});